...
def get_rays_from_pixels(xy_grid, image_size, camera):
    ...
    # rays_o = camera.get_camera_center() + torch.cat([xy_grid, torch.zeros_like(ndc_points[..., -1:])],dim=-1)
    # rays_o = camera.T + world_points
    rays_o =camera.get_camera_center().repeat(H * W, 1)

    # TODO (1): Get normalized ray directions
    rays_d = world_points - rays_o
    rays_d = rays_d / torch.norm(rays_d, dim=1, keepdim=True, p=2)
